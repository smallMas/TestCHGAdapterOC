...
+ (SecKeyRef)addPublicKey:(NSString *)key{
	...
	data = [RSAHandle stripPublicKeyHeader:data];
	if(!data){
		return nil;
	}
...
+ (SecKeyRef)addPrivateKey:(NSString *)key{
	...
	// This will be base64 encoded, decode it.
	NSData *data = base64_decode(key);
	data = [RSAHandle stripPrivateKeyHeader:data];
	if(!data){
		return nil;
	}
...
+ (NSData *)encryptData:(NSData *)data privateKey:(NSString *)privKey{
	...
	SecKeyRef keyRef = [RSAHandle addPrivateKey:privKey];
	if(!keyRef){
		return nil;
	}
	return [RSAHandle encryptData:data withKeyRef:keyRef isSign:YES];
}
...
+ (NSData *)decryptData:(NSData *)data privateKey:(NSString *)privKey{
	...
	return [RSAHandle decryptData:data withKeyRef:keyRef];
}
...
+ (NSData *)encryptData:(NSData *)data publicKey:(NSString *)pubKey{
	...
	SecKeyRef keyRef = [RSAHandle addPublicKey:pubKey];
	if(!keyRef){
		return nil;
	}
